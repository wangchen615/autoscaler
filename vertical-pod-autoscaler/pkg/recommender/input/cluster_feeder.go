	...
	for _, vpaCRD := range allVpaCRDs {
		...
		if len(vpaCRD.Spec.RecommenderName) > 0{
			if !findElement(vpaCRD.Spec.RecommenderName, currentRecommenderName) {
				...
				klog.V(3).Infof("The vpaCRD %s recommender name %v includes the current recommender's name %v, so the vpaCRD is selected", vpaCRD.Name, vpaCRD.Spec.RecommenderName, currentRecommenderName)
			}
		} else {
